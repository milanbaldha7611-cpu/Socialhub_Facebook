...
if (isset($_SESSION['is_loggedin']) && $_SESSION['is_loggedin']) {
    ...
    // Admin Chat Unlock Session Timeout (10 minutes = 600 seconds)
    if (isset($_SESSION['chat_unlocked']) && $_SESSION['chat_unlocked']) {
        if (isset($_SESSION['chat_unlock_time']) && (time() - $_SESSION['chat_unlock_time'] > 600)) {
            unset($_SESSION['chat_unlocked']);
            unset($_SESSION['chat_unlock_time']);
        } else {
            $_SESSION['chat_unlock_time'] = time(); // Refresh activity timer
        }
    }
    ...
    <body>
        <header class="admin-header">
            <div class="container-fluid admin-nav-content">
                ...
                <div class="d-flex align-items-center">
                    <span class="admin-greeting"><i class="fas fa-crown text-warning me-2"></i>Welcome,
        ...
        <div class="container-fluid px-md-5 mt-5" style="flex: 1;">
            ...
            <ul class="nav nav-pills justify-content-center mb-5" id="adminTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="users-tab" data-bs-toggle="pill" data-bs-target="#users-section"
                        type="button" role="tab"><i class="fas fa-users me-2"></i>Users</button>
                </li>
                <li class="nav-item mx-3" role="presentation">
                    <button class="nav-link" id="posts-tab" data-bs-toggle="pill" data-bs-target="#posts-section"
                        type="button" role="tab"><i class="fas fa-images me-2"></i>Posts</button>
                </li>
                <li class="nav-item me-3" role="presentation">
                    <button class="nav-link" id="stories-tab" data-bs-toggle="pill" data-bs-target="#stories-section"
                        type="button" role="tab"><i class="fas fa-history me-2"></i>Stories</button>
                </li>
                <li class="nav-item me-3" role="presentation">
                    <button class="nav-link" id="chats-tab" data-bs-toggle="pill" data-bs-target="#chats-section"
                        type="button" role="tab"><i class="fas fa-comments me-2"></i>Chats</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="blocked-tab" data-bs-toggle="pill" data-bs-target="#blocked-section"
                        type="button" role="tab"><i class="fas fa-user-slash me-2"></i>Blocked Status</button>
                </li>
            </ul>
            ...
            <div class="tab-content" id="adminTabsContent">
                                ...
                                <table class="table custom-table" id="postsTable">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            if (!$user_arr)
                                                continue; // Remove entries with Unknown User
                ...
                <div class="tab-pane fade" id="chats-section" role="tabpanel">
                    <div class="admin-container mt-0 chat-lock-container">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h2 class="admin-title mb-0"><i class="fas fa-comments"></i> Chat Management</h2>
                            <?php if (isset($_SESSION['chat_unlocked']) && $_SESSION['chat_unlocked']): ?>
                                <button class="btn btn-sm btn-outline-danger" onclick="lockAdminChats()" style="border-radius: 8px;">
                                    <i class="fas fa-lock me-1"></i> Lock Chats
                                </button>
                            <?php endif; ?>
                        </div>

                        <?php 
                        $is_locked = !isset($_SESSION['chat_unlocked']) || !$_SESSION['chat_unlocked'];
                        if ($is_locked): 
                        ?>
                            <div class="chat-lock-overlay">
                                <i class="fas fa-user-shield fa-4x mb-4" style="color: #3b82f6; opacity: 0.8;"></i>
                                <h3 class="mb-3">Secure Chat Access</h3>
                                <p class="text-muted mb-4">Sensitive data is protected. Access requires authorization.</p>
                                <button class="btn btn-primary px-5 py-2" onclick="showUnlockModal()" style="border-radius: 12px; font-weight: 600; box-shadow: 0 4px 15px rgba(59, 130, 246, 0.4);">
                                    <i class="fas fa-key me-2"></i> Unlock Chats
                                </button>
                            </div>
                        <?php endif; ?>

                        <div class="chat-content-wrap <?php echo $is_locked ? 'is-locked' : ''; ?>">
                    ...
                    <div class="modal-body">
                        <div class="text-center mb-3">
                            ...
                            <label class="btn btn-sm btn-outline-primary mt-2">
                                ...
                                <input type="file" name="user_image" class="edit-image-input"
                                    data-target="#user_image_preview" hidden accept="image/*">
                            </label>
                        </div>
                ...
                <form action="user.php" method="POST" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="post_caption" id="edit_post_caption" class="form-control" rows="4"
                                required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                ...
                <form action="user.php" method="POST" enctype="multipart/form-data" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="post_image" class="form-control edit-image-input"
                                data-target="#post_media_preview" accept="image/*" required>
                ...
                <form action="user.php" method="POST" enctype="multipart/form-data" class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="image-preview-wrapper mb-3" id="story_media_preview_container">
                            ...
                            <video id="story_video_preview" controls
                                style="display:none; max-width: 100%; max-height: 100%;"></video>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Select New Media</label>
                            <input type="file" name="story_media" class="form-control story-edit-input"
                                data-img-target="#story_media_preview" data-vid-target="#story_video_preview"
                ...
                <form action="user.php" method="POST" class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="msg_content" id="edit_msg_content" class="form-control" rows="4"
                                required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
        ...
        <!-- Chat Unlock Modal -->
    <div class="modal fade" id="chatUnlockModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="background: #1e293b; border: 1px solid rgba(255,255,255,0.1); border-radius: 20px;">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title w-100 text-center pt-3">
                        <i class="fas fa-unlock-alt text-primary mb-2 fa-2x d-block"></i>
                ...
                <div class="modal-body p-4">
                    <form id="adminUnlockForm">
                        <div class="mb-4">
                            <label class="form-label text-muted small">Enter Security Password</label>
                            <div class="position-relative">
                                <input type="password" id="adminChatPass" class="form-control" placeholder="••••••••" 
                                    style="background: rgba(15, 23, 42, 0.6); color: white; border: 1px solid rgba(255,255,255,0.1); padding: 12px 40px 12px 15px; border-radius: 12px;">
                                <i class="fas fa-eye position-absolute chat-pass-toggle" onclick="toggleChatPassVisibility()"
                                    style="right: 15px; top: 50%; transform: translateY(-50%); cursor: pointer; color: #64748b;"></i>
                            </div>
                            <div id="unlockError" class="text-danger small mt-2 d-none">
                                <i class="fas fa-exclamation-circle me-1"></i> <span id="errorMsg"></span>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary w-100 py-2" style="border-radius: 12px; font-weight: 600;">
                            Verify & Unlock
                        </button>
                    </form>
                </div>
    ...
    <script>
        function showUnlockModal() {
            const modal = new bootstrap.Modal(document.getElementById('chatUnlockModal'));
            modal.show();
        }

        function toggleChatPassVisibility() {
            const input = document.getElementById('adminChatPass');
            const icon = document.querySelector('.chat-pass-toggle');
        ...
        document.getElementById('adminUnlockForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    location.reload(); // Reload to apply server-side session change
                } else {
        ...
        function lockAdminChats() {
            // Simply call a script to unset session or clear on client side
            // For now we just unset it via a simple GET parameter or another fetch
            fetch('verify_admin_chat.php?action=lock')
